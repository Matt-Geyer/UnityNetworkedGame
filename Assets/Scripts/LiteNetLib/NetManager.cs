...
namespace LiteNetLib
{
    ...
    public interface IAsyncUdpMessageSender
    {
        void Send(byte[] buffer, int offset, int size, IPEndPoint remoteEndpoint, UdpSendType sendType);
    }
    ...
    public sealed class NetManager : INetSocketListener, IConnectionRequestListener, IEnumerable<NetPeer>
    {
        ...
        /// </summary>
        public int LocalPort { get { throw new NotImplementedException(); } }
        ...
        public NetManager(INetEventListener listener)
        {
            ...
            //TODO: NatPunch needs a ringbuf ref
        ...
        public bool SendDiscoveryRequest(byte[] data, int start, int length, int port)
        {
            ...
            bool result = false;// _socket.SendBroadcast(packet.RawData, 0, packet.Size, port);
            NetPacketPool.Recycle(packet);
            return result;
        }
